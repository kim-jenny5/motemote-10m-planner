...
import { Step, steps } from '@/types/welcome';

const isValidEmail = (value: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);

export default function Welcome() {
  const [plannerColor, setPlannerColor] = useState<string | undefined>(undefined);
  const [step, setStep] = useState<Step>('name');

  useEffect(() => {
    const color = sessionStorage.getItem('plannerColor') || undefined;
  ...
  const emailError = step === 'email' && emailTouched && !isValidEmail(email);
  ...
  return (
    <div className='page-container flex max-w-5xl flex-col items-center justify-center'>
      <BlobBackground color={plannerColor} />
              ...
              {step === 'name' && (
                <>
                  ...
                  <input
                    ...
                    className='w-full rounded-sm bg-white px-4 py-3 text-sm shadow-md focus:ring-2 focus:ring-stone-700 focus:outline-none'
                  />
                </>
              )}
              {step === 'email' && (
